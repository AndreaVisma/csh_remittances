...
df = pd.read_csv("C:\\Data\\my_datasets\\remittances_results\\yearly_flows.csv")
df.date = pd.to_datetime(df.date)
df["year"] = df.date.dt.year
df_out = (df[["year", "destination", "sim_remittances"]].groupby(["year", "destination"]).sum()
         .reset_index().rename(columns = {'destination' : 'country', 'sim_remittances' : 'outflow'}))
df_in = (df[["year", "origin", "sim_remittances"]].groupby(["year", "origin"]).sum()
         .reset_index().rename(columns = {'origin' : 'country', 'sim_remittances' : 'inflow'}))

df_rem = df_in.merge(df_out, on = ['country', 'year'])
print(set(df_rem.country) - set(dict_names.keys()))
df_rem_2019 = df_rem[df_rem.year == 2019]

#### population
df_pop = pd.read_excel("c:\\data\\population\\population_by_country_wb_clean.xlsx")
df_pop = df_pop[df_pop.year == 2019]
df_rem_2019 = df_rem_2019.merge(df_pop, on = ['country', 'year'])
...
df_rem_2019 = df_rem_2019.merge(df_gdp, on = ['country', 'year'])

##### calculate ratios
df_rem_2019['in_per_capita'] = df_rem_2019['inflow'] / df_rem_2019['population']
df_rem_2019['out_per_capita'] = df_rem_2019['outflow'] / df_rem_2019['population']
df_rem_2019['gdp_per_capita'] = df_rem_2019['gdp'] / df_rem_2019['population']

df_rem_2019['pct_in'] = 100 * df_rem_2019['in_per_capita'] / df_rem_2019['gdp_per_capita']
df_rem_2019['pct_out'] = 100 * df_rem_2019['out_per_capita'] / df_rem_2019['gdp_per_capita']
df_rem_2019 = df_rem_2019[df_rem_2019.gdp>1e10]
...
df_rem_2019 = df_rem_2019.merge(income_class[['country', 'group', 'Region']], on = 'country', how = 'left')

##################
## chart with only grey dots
sns.set_theme(style="white")
max_val_size = df_rem_2019.population.max() / df_rem_2019.population.min()
df_rem_small = df_rem_2019[(df_rem_2019.pct_in < 10) & (df_rem_2019.pct_out < 2)]

fig, ax = plt.subplots(figsize = (12,9))
sns.scatterplot(x="pct_out", y="pct_in", size="population",
            sizes=(100, max_val_size), alpha=.8, hue = 'country',
...
plt.legend()
plt.show(block = True)
fig.savefig('.\plots\\for_paper\\ESTIMATE_multiplier_effect_scatter_grey_inner.png')

df_rem_big = df_rem_2019[(df_rem_2019.pct_in >= 10) | (df_rem_2019.pct_out >= 2)]
...
plt.legend()
plt.show(block = True)
fig.savefig('.\plots\\for_paper\\ESTIMATE_multiplier_effect_scatter_grey_outer.png')
