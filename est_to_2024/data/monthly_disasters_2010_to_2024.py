...
## world population
df_pop_country = pd.read_excel("c:\\data\\population\\population_wb_to_2024.xls",
                   skiprows = 3)
df_pop_country.set_index('Country Name', inplace=True)
df_pop_country = df_pop_country.iloc[:, 3:].reset_index().rename(columns={df_pop_country.index.name:'country'})

df_pop_country = pd.melt(df_pop_country, id_vars='country', value_vars=df_pop_country.columns[1:], var_name='year', value_name='population')
print(set(df_pop_country.country) - set(dict_names.keys()))
df_pop_country['country'] = df_pop_country['country'].map(dict_names)
df_pop_country.dropna(inplace = True)
df_pop_country['year'] = df_pop_country['year'].astype(int)

#read the EM-DAT data
emdat = pd.read_excel("c:\\data\\natural_disasters\\emdat_2010_2026.xlsx")
emdat = emdat[(emdat["Start Year"] >= 2009) &
...
# Save
df_nat_pivot.to_pickle("C:\\Data\\my_datasets\\monthly_disasters_with_lags_2010_2024.pkl")
