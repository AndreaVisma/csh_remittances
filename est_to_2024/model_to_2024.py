...
## Diaspora numbers
diasporas_file = "C:\\Data\\migration\\bilateral_stocks\\interpolated_stocks_and_dem_factors_2010_to_2024.pkl"
df = pd.read_pickle(diasporas_file)
df = df.dropna()

###gdp to infer remittances amount
df_gdp = pd.read_pickle("c:\\data\\economic\\gdp\\annual_gdp_per_capita_splined_2010_to_2024.pkl")
...
def simulate_remittances_agresti_coull(df_countries, height, shape, shift, rem_pct, disasters = True):
    ...
    df_countries['probability'] = 1 / (1 + np.exp(-df_countries["theta"]))
    df_countries.loc[df_countries.nta == 0, 'probability'] = 0
    df_countries['standard_error'] = np.sqrt((df_countries['probability'] * (1 - df_countries['probability'])/
                                              (df_countries['n_people'])))
    df_countries["senders_95"] = ((df_countries['probability'] + 1.96 * df_countries['standard_error']) * df_countries["n_people"])
    df_countries["senders_05"] = ((df_countries['probability'] - 1.96 * df_countries['standard_error']) * df_countries[
        "n_people"])

    df_countries['sim_senders'] = (df_countries['probability'] * df_countries['n_people']).astype(int)

    df_countries['sim_remittances'] = df_countries['sim_senders'] * df_countries['rem_amount']
    df_countries['sim_remittances_95'] = df_countries['senders_95'] * df_countries['rem_amount']
    df_countries['sim_remittances_05'] = df_countries['senders_05'] * df_countries['rem_amount']

    remittance_per_period = (df_countries.groupby(['date', 'origin', 'destination'])
         [['sim_remittances', 'sim_senders', "senders_95", 'sim_remittances_95', "senders_05", 'sim_remittances_05']].sum().reset_index())

    return remittance_per_period

def simulate_remittances_exact_dist(df_countries, height, shape, shift, rem_pct, disasters = True):
    ...
    # df_countries['standard_error'] = np.sqrt((df_countries['probability'] * (1 - df_countries['probability'])/
    #                                           (df_countries['n_people'])))
    df_countries["senders_05"] = (binom.ppf(0.05 / 2, df_countries['n_people'], df_countries['probability']))
    df_countries["senders_95"] = (binom.ppf(1 - (0.05 / 2), df_countries['n_people'], df_countries['probability']))
...
df_results.to_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\all_flows_simulations_with_disasters_2010_2024.parquet")
# # WITHOUT
print("simulating NO disasters ...")
df_results = simulate_remittances(df, height, shape, shift, rem_pct, disasters = False)
df_results.to_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\all_flows_simulations_without_disasters_2010_2024.parquet")
# # DROUGHT
...
df_results.to_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\2010_2024_drought.parquet")
# FLOOD
print("simulating floods ...")
df_results = simulate_remittances_specific_disaster(df, height, shape, shift, rem_pct, disaster="Flood")
df_results.to_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\2010_2024_floods.parquet")
# WITHOUT
print("simulating storms ...")
df_results = simulate_remittances_specific_disaster(df, height, shape, shift, rem_pct, disaster="Storm")
df_results.to_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\2010_2024_storms.parquet")
# WITHOUT
...
df_results.to_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\2010_2024_earthquakes.parquet")

#################
# plot
#################

df_with = pd.read_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\all_flows_simulations_with_disasters_2010_2024.parquet")
df_without = pd.read_parquet("C:\\git-projects\\csh_remittances\\general\\results_plots\\all_flows_simulations_without_disasters_2010_2024.parquet")
...
# df_all_group.shape
df_all_group.rename(columns = {'origin' : 'recipient', 'destination' : 'sender'}, inplace = True)
df_all_group.to_csv("C:\\git-projects\\csh_remittances\\general\\results_plots\\yearly_flows_2010_2024.csv", index = False)
...
df_out = df_out[(df_out.year > 2009)]
...
df_in_country = df_in_country[(df_in_country.year > 2009)]
...
######## plot for the paper
df_mean = df_by_country_dest[df_by_country_dest.year < 2025].copy()
df_mean = df_mean[["country", "sim_remittances_with", "inflow"]].groupby(["country"]).mean().reset_index()

df_mean = df_mean[
    (df_mean['inflow'] > 0.001) &
    (df_mean['sim_remittances_with'] > 0.001)]

x_log = np.log10(df_mean['inflow'])
y_log = np.log10(df_mean['sim_remittances_with'])
...
unique_countries = df_mean['country'].unique()

# 4. Scatter Plot (Iterate to handle coloring and legend automatically)
ax.scatter(df_mean['inflow'], df_mean['sim_remittances_with'])

# 5. Set Axes to Logarithmic Scale
ax.set_xscale('log')
ax.set_yscale('log')

# 6. Add 45-degree y = x line (must use the same range in both scales)
# Determine the global limits for the diagonal line
min_val = min(df_mean['inflow'].min(), df_mean['sim_remittances_with'].min())
max_val = max(df_mean['inflow'].max(), df_mean['sim_remittances_with'].max())
...
ax.plot(
    ...
    color='black',
    alpha=0.8,
    zorder=0, # Ensure line is behind the scatter points
    label='y = x'
)

# 7. Add Labels and Title
ax.set_xlabel('Est remittances World Bank (billion)', fontsize=12)
ax.set_ylabel('Sim remittances my model (billion)', fontsize=12)

# 9. Final Layout Tweaks
# ax.grid(True, which="both", ls="--", color='lightgray', alpha=0.6) # Add gridlines

plt.savefig("C:\\git-projects\\csh_remittances\\plots\\for_paper\\latest_results\\comparison_KNOMAD_to_2024.svg", bbox_inches = "tight")
plt.show(block = True)
########################
...
fig.update_layout(
    ...
    title='Comparison of remittances inflow (KNOMAD vs. our model, total inflow from 2010 to 2024)'
)
fig.write_html("C:\\git-projects\\csh_remittances\\plots\\for_paper\\latest_results\\comparison_KNOMAD_to_2024.html")
fig.show()
