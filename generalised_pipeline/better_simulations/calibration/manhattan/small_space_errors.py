...
df1 = pd.read_pickle('all_results_0207.pkl')
df2 = pd.read_pickle('all_results_0207_1.pkl')
res = pd.concat([df1, df2])
res.rename(columns = {'hsift' : 'shift'}, inplace = True)

##############

alpha = 0.8

param_names = ['nta','asy','gdp','height','shape', 'shift','constant']
res.loc[~res[param_names].duplicated(), 'run_nr'] = [x for x in range(100)]
res['run_nr'].ffill(inplace = True)
res['error'] = (res['sim_remittances'] - res['remittances']) / 1e9
res['error_rel_obs'] = (abs(res['sim_remittances'] - res['remittances']) / (res['remittances'] + 1)) / 1e6
res['hybrid_error'] = alpha * res['error'] + (1 - alpha) * res['error_rel_obs']

df_sum = res[['run_nr', 'error', 'error_rel_obs', 'hybrid_error']].groupby('run_nr').agg(
    ...
    sum_squared_hyb_err = pd.NamedAgg(column="hybrid_error", aggfunc=lambda x: np.sum(np.square(x)))
).reset_index()
df_sum = df_sum.merge(res[~res.run_nr.duplicated()][['run_nr'] + param_names], how = 'left')
df_sum.sort_values('sum_squared_hyb_err', inplace = True)

df_sum.to_excel("C://Users//Andrea Vismara//Downloads//sim_errors_0207.xlsx", index = False)

##############
# try to optimise

# parameter columns
param_cols = ["nta", "asy", "gdp", "height", "shape", "shift", "constant"]

# choose the error metric you want to model:
target_col = "squared_err"
...
def error_surrogate(nta, asy, gdp, height, shape, shift, constant):
    """
    Predicts the sum_squared_err for given parameters.
    Returns a scalar float.
    """
    xp = np.array([[nta, asy, gdp, height, shape, shift, constant]])
    return float(rf.predict(xp))

# example usage:
print(error_surrogate(2.5, -7.1,  9.2, 0.3, 0.1, 1, -0.5))
